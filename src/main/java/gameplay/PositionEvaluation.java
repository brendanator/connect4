...
public class PositionEvaluation {
  ...
  private void evaluatePosition() {
    ...
    if (redWin > 0) {
      ...
      positionValue = 0 + doubleThreat.score() +
                      zugwang.score()
                      // TODO Only include horizontal and diagonal threats?
                      + cardinality(redThreats) * 100 -
                      cardinality(yellowThreats) * 100
  ...
  private void analyzePosition() {
    ...
    for (int i = 0; i < PositionConsts.HORIZONTAL_ODD_FOURS_IN_A_ROW.length;
         i++) {
    ...
    for (int i = 0; i < PositionConsts.HORIZONTAL_EVEN_FOURS_IN_A_ROW.length;
         i++) {
    ...
    yellowThreats =
        yellowFourInARowChances ^ (yellowPieces & yellowFourInARowChances);
    ...
    for (int i = 0; i < PositionConsts.WIDTH; i++) {
      ...
      for (int j = 0; j < PositionConsts.HEIGHT; j++) {
        ...
        if (redThreat && yellowThreat) {
          if (lowestRedThreat == PositionConsts.HEIGHT &&
              lowestYellowThreat == PositionConsts.HEIGHT) {
            if (oddRow) {
              sharedOddThreats += 1;
            } else {
        ...
        if (redThreat) {
          ...
          if (existingRedThreat && lowestYellowThreat >= j) {
            lowestRedDoubleThreat = Math.min(
                lowestRedDoubleThreat,
                (j - 1 - cardinality(pieces & PositionConsts.COLUMNS[i])));
          } else {
        ...
        if (yellowThreat) {
          ...
          if (existingYellowThreat && lowestRedThreat >= j) {
            lowestYellowDoubleThreat = Math.min(
                lowestYellowDoubleThreat,
                (j - 1 - cardinality(pieces & PositionConsts.COLUMNS[i])));
          } else {
  ...
  private void analyzeDoubleThreat(int lowestRedDoubleThreat,
    ...
    if (lowestRedDoubleThreat == PositionConsts.HEIGHT &&
        lowestYellowDoubleThreat == PositionConsts.HEIGHT)
  ...
  private void analyzeZugwang(int sharedOddThreats, int redOddThreats,
                              int yellowOddThreats,
                              int redOddAboveYellowOddThreats,
                              int yellowOddAboveRedOddThreats,
                              boolean yellowEvenThreats) {
    ...
    if (!yellowEvenThreats && (yellowOddThreats >= redOddThreats) &&
        (redOddThreats >= yellowOddThreats - 1) &&
        (redOddAboveYellowOddThreats == 0)) {
      zugwang = Zugwang.NONE;
    } else if ((((redOddThreats > yellowOddThreats) ||
                 (yellowOddThreats == 1 && yellowOddAboveRedOddThreats == 0)) &&
                (redOddThreats + redOddAboveYellowOddThreats >=
                 yellowOddThreats + yellowOddAboveRedOddThreats)) ||
               (redOddThreats + redOddAboveYellowOddThreats >
                yellowOddThreats + yellowOddAboveRedOddThreats)) {
      zugwang = Zugwang.RED;
    } else {
  ...
  public String toString() {
    return String.format(
        "Value: %d  DoubleThreat: %s  Zugwang: %s  Threats: %d, %d  Minor Threats: %d, %d  Blockers: %d %d",
        positionValue, doubleThreat, zugwang, cardinality(redThreats),
        cardinality(yellowThreats), redMinorThreatCount, yellowMinorThreatCount,
        redBlockers, yellowBlockers);
  }

  public int getPositionValue() { return positionValue; }

  public long getRedWin() { return redWin; }

  public long getYellowWin() { return yellowWin; }
}
