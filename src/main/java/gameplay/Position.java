...
public class Position {
  ...
  public Position() { this(Turn.RED, 0, 0, 0); }

  private Position(Turn turn, long redPieces, long yellowPieces,
                   long zobristHash) {
  ...
  public long zobristHashCode() { return zobristHash; }

  private void evaluatePosition() {
    positionValue = new PositionEvaluation(turn, redPieces, yellowPieces)
                        .getPositionValue();
  }
  ...
  public boolean isGameOver() {
    return Math.abs(positionValue) == PositionConsts.WIN_SCORE ||
        cardinality(redPieces + yellowPieces) == PositionConsts.SIZE;
  }
  ...
  private Position getNewPosition(int moveColumn, int moveRow) {
    long newZobristHash =
        ZobristHashing.getMoveHash(zobristHash, turn, moveColumn, moveRow);
    ...
    newPosition =
        new Position(newTurn, newRedPieces, newYellowPieces, newZobristHash);
